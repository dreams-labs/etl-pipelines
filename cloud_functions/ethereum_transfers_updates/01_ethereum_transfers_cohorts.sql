...
insert into etl_pipelines.ethereum_transfers_cohorts (
    ...
    select 5 as cohort_number
    ,'coingecko_all_coins rank up to 14000 as of jan 2 2025' as cohort_description
    ,c.coin_id
    ,c.coingecko_id
    ,c.chain
    ,c.address
    ...
    ,c.decimals
    from core.coins c
    left join etl_pipelines.ethereum_transfers_cohorts etc on etc.coin_id = c.coin_id
    left join (
        select token_address
        from etl_pipelines.ethereum_net_transfers t
        group by 1
    ...
    -- only include coins that aren't already in the eth transfers table or in a cohort
    and etc.coin_id is null
    and t.token_address is null
