...
def test_add_first_price_info_standard():
    """
    Tests add_first_price_info with standard scenario where price data starts after some null values.

    Validates that:
    1. First non-null price date is correctly identified for each coin
    2. First price value is correctly captured
    3. Original data is preserved
    4. New columns are properly added with correct values
    """
    # Create test data with a mix of null and non-null prices
    input_df = pd.DataFrame({
        'coin_id': ['coin1'] * 6 + ['coin2'] * 6,
        'wallet_address': ['wallet1'] * 6 + ['wallet1'] * 6,
        'date': pd.to_datetime([
            # Coin1 dates - prices start on Jan 3
            '2024-01-01', '2024-01-02', '2024-01-03', '2024-01-04', '2024-01-05', '2024-01-06',
            # Coin2 dates - prices start on Jan 4
            '2024-01-01', '2024-01-02', '2024-01-03', '2024-01-04', '2024-01-05', '2024-01-06'
        ]),
        'net_transfers': [10.0] * 12,  # Dummy values
        'balance': [100.0] * 12,       # Dummy values
        'price': [
            # Coin1 prices: null, null, 1.0, 1.1, 1.2, 1.1
            np.nan, np.nan, 1.0, 1.1, 1.2, 1.1,
            # Coin2 prices: null, null, null, 2.0, 2.1, 2.2
    ...
    # Execute function
    result_df = cwp.add_first_price_info(input_df)

    # Verify results
    # Check that all original records are preserved
    assert len(result_df) == len(input_df), "Should preserve all records"
    ...
    assert all(col in result_df.columns for col in input_df.columns), \
        "All original columns should be present"

    # Compare original data between input and result
    # We create filtered versions of both dataframes with only the original columns
    result_orig_cols = result_df[input_df.columns]

    # Compare numeric columns
    numeric_cols = ['net_transfers', 'balance', 'price']
    for col in numeric_cols:
        assert np.allclose(result_orig_cols[col], input_df[col],
                          equal_nan=True), f"Original {col} data was modified"

    # Compare date column
    assert (result_orig_cols['date'] == input_df['date']).all(), "Date data was modified"

    # Compare wallet_address column
    assert (result_orig_cols['wallet_address'] ==
            input_df['wallet_address']).all(), "Wallet address data was modified"

    # Check first price info for Coin1
    ...
    # Check first price info for Coin2
...
def test_add_first_price_info_staggered_starts():
    ...
    Tests both recent and historical first prices, different patterns of null values,
    and ensures each coin's first price info is independently tracked.

    Validates that:
    1. First prices are correctly identified regardless of when they appear
    ...
    """
    # Create test data with three coins having staggered price starts
    input_df = pd.DataFrame({
    ...
    # Define expected first price info for each coin
